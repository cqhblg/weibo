...
object SessionBase{
  ...
  val SessionTypeKey = "STKey"
    ...
    object SessionKeys {
      ...
      //加入博客key
      val blogid="todos2018_blogid"
      val blogauthor="todos2018_blogauthor"
    }
    ...
    case class BlogSession(
                          blogInfo:BlogBaseInfo,
                          time:Long
                          ){
      def toSessionMap:Map[String,String]={
        Map(
          SessionTypeKey -> SessionKeys.sessionType,
          SessionKeys.blogid ->blogInfo.id.toString,
          SessionKeys.timestamp -> time.toString
        )
      }
    }
...
trait SessionBase extends SessionSupport with ServiceUtils{
  ...
  implicit class SessionTransformer(sessionMap: Map[String, String]) {
    ...
    //blogsession
    def blogSession:Option[BlogSession] = {
      try {
        if (sessionMap.get(SessionTypeKey).exists(_.equals(SessionKeys.sessionType))) {
//          if(sessionMap(SessionKeys.timestamp).toLong - System.currentTimeMillis() > timeout){
//            None
//          }else {
            Some(BlogSession(
              BlogBaseInfo(sessionMap(SessionKeys.blogid).toInt),
              sessionMap(SessionKeys.timestamp).toLong
            ))
//          }
        } else {
...
//    log.info(s"Access uri: ${ctx.request.uri} from ip ${ctx.request.uri.authority.host.address}.")
  ...
  //optionalBlogSession
  protected val optionalBlogSession: Directive1[Option[BlogSession]] = optionalSession.flatMap {
    case Right(sessionMap) => BasicDirectives.provide(sessionMap.blogSession)
    case Left(error) =>
      log.debug(error)
      BasicDirectives.provide(None)
  }
  ...
  //博客
  def blogAuth(f:BlogBaseInfo=>server.Route)  = loggingAction{ ctx =>
    optionalBlogSession{
      case Some(session) =>
        f(session.blogInfo)
      case None =>
